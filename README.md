...
### Экспорт/резервное копирование БД

Добавлен endpoint для экспорта дампа базы данных (SQL):

- GET `/api/admin/export-db` — возвращает файл `backup.sql` с дампом всей базы. Запрос работает только с admin токеном.

Рекомендуется настроить автоматические бэкапы (GitHub Actions) и хранить их как артефакты сборок или загружать в облачное хранилище (S3).

#### GitHub Actions: примечание
Чтобы настроить бэкап через GitHub Actions, добавьте секрет репозитория `ADMIN_PASSWORD` (тот же пароль, что в `.env`) — workflow будет использовать его для получения admin-токена и скачивания дампа.

1. В GitHub → репозиторий → Settings → Secrets and variables → Actions → New repository secret
   - Name: `ADMIN_PASSWORD`
   - Value: тот же пароль администратора (из `.env`)
2. После пуша workflow будет появляться в Actions → `DB Backup` и запускаться ежедневно (и вручную через workflow_dispatch).
3. Бэкапы будут доступны в разделе Actions → выбранный запуск → Artifacts → `sqlite-backup`.
